...
這部分的用法可以參考影片
```{r}
df2 = subset(df[,c(1,4,5,6,8,2,3,7,9)], size >= 20 & size <= 1000)
```


```{r MotionChart1, results='asis', tidy=FALSE}
M = gvisMotionChart(
  df2, "kg", "dummy", 
  options=list(width=720, height=600))
plot(M)
```
...
df2 = subset(df, group_size >= 20 & group_size <= 1200)
```

# Google Motion Chart
```{r MotionChart2, results='asis', tidy=FALSE}
M = gvisMotionChart(
  df2, "kg", "dummy", 
  options=list(width=720, height=600) ) 
plot(M)
